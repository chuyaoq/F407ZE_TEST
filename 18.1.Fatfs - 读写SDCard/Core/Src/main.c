...
void sd_mount(void)
{
    ...
    FRESULT res;            // 状态
    UINT bw;                // 实际写入字节数
    char write_buf[] = "Stars Pi SD Card FATFS Test\r\n"; 

    retSD = f_mount(&SDFatFS, "0:", 1);   //挂载文件
    
    if (retSD == FR_NO_FILESYSTEM)
    {
        printf("SD NO FATFS,Initing\r\n");
        static uint8_t aMountBuffer[4096];
        retSD = f_mkfs("0:", FM_FAT32, 0, aMountBuffer, sizeof(aMountBuffer));
    ...
    {
        if (SDFatFS.fs_type == 0x03)
            printf("SD FATFS :FAT32\n");
        if (SDFatFS.fs_type == 0x04)
